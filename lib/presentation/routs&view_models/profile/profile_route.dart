...
class _ProfileRouteState extends State<ProfileRoute> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<EventsBloc , AppStates>(builder: (context , state)=>Scaffold(
      body: stackBackGroundManager(isDark: false, otherWidget: screenWidgets(bloc)),
    ), listener: (context , state){
      if(state is LogoutState){
        SharedPref.prefs.remove(GeneralStrings.currentUser);
        Navigator.pushReplacementNamed(context, RouteStringsManager.questionRoute,);
      }
    });
  }
     ...
     children: [
       ...
       Row(
         children: [
           ...
           } , child: Container(height: 70, width: 120, color: Colors.green ,)),
           GestureDetector(onTap: (){
             bloc.add(ChangeColorModeEvent(AppColorsTheme.purple));
           } , child: Container(height: 70, width: 120, color: Colors.purple ,)),
           GestureDetector(onTap: (){
             bloc.add(ChangeColorModeEvent(AppColorsTheme.blue));
           } , child: Container(height: 70, width: 120, color: Colors.blue ,)),
           GestureDetector(onTap: (){
             navigateTo(context, RouteStringsManager.takeUserDetailsRoute);
           } , child: Container(height: 70, width: 120, color: Colors.red ,)),
